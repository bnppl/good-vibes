...
Two problems share this session because they are the same problem. **Characterization tests** pin behavior in legacy code before an agent is allowed to touch it — they answer the question "what does this actually do, before I let something rewrite it?" The **handoff problem** asks the harder follow-up: where do those tests, the scenarios from [[bdd-for-agents]], the contracts from [[contract-testing]], and the fitness functions from [[fitness-functions]] live so that the *next* agent session — fresh context, no memory of yours — discovers them?
...
2. **Instruction-layer pointers.** `AGENTS.md` / `CLAUDE.md` names the verification surface explicitly: which commands to run, which directories hold which artifacts, what thresholds matter. This is the [[../context-engineering/instruction-layer]] doing exactly the job it was designed for: telling a fresh session what the project considers true. The orchestration layer ([[../context-engineering/orchestration-layer]]) is what makes sure the instruction file actually loads in the first prompt; co-location is what makes the artifacts findable once the agent goes looking. They are complementary, not redundant — see [[../context-engineering/agentic-dev]] for the broader pattern.
...
This module took the verification habits from Module 3 — the per-session loop of [[agentic-tdd]] and [[comprehension-debt]] — and extended them into **artifacts that span sessions**. Cross-session regression suites ([[cross-session-regression]]). Behavior scenarios in shared `features/` directories. Domain boundaries that survive because [[ddd-boundaries]] are encoded in [[fitness-functions]]. Contracts that don't drift because [[contract-testing]] catches the drift. Tests whose own value is checked by [[test-the-tests]].
