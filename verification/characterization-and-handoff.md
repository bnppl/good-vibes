...
The **[vltansky agents-md-evals](https://github.com/vltansky/agents-md-evals)** findings sharpen the picture further: in controlled testing, 25 of 26 assertions passed identically with and without a 755-line instruction file. Most AGENTS.md files are 80–95% redundant because of the **codebase-teaches-patterns effect** — well-structured codebases make most instruction rules redundant. The model reads `package.json`, scans existing components, follows import patterns, and matches conventions automatically. Realistic improvement deltas from AGENTS.md are +3–10% for well-structured projects; the gains come almost entirely from the rules the codebase *cannot* teach: coupling decisions, workflow requirements, decision-making preferences, and domain context that isn't visible in any single file.

The **Augment Code benchmark evaluation** (from their system prompt analysis comparing identical models across tools) adds a sharper frame: identical tasks on identical models scored 17 problems apart depending on context engineering quality — a gap equivalent in magnitude to a model generation upgrade. Their framing: "a good AGENTS.md is a model upgrade." The mechanism is the same as the Vercel eval: right information at the right time lets the model skip re-deriving known constraints and spend that capacity on the actual work. **[Sebastian Raschka (rasbt)](https://x.com/rasbt)** provides the useful counterpoint: in his testing, the majority of rules in typical AGENTS.md files are things capable models would already do without instruction. The rules that carry genuine information — cross-session constraints, domain-specific decisions, things the model cannot infer from the codebase — are often a small fraction of the total file. The practical synthesis: the "model upgrade" framing applies, but *only* to the rules with genuine information content. The rest trains the agent to skim. Trim to the rules that would cause a real failure if missing.
...
The **vltansky agents-md-evals** tool operationalizes this with a "Keep Test" for each rule: a rule survives if it passes at least one criterion: **Specificity** (it says something precise, not generic), **Behavior change** (it actually changes what the agent does), **Surprise** (the agent would do the wrong thing without it), **Pain** (removing it has caused incidents), **Frequency** (it applies to something the agent does regularly), or **No feedback loop** (it covers a constraint the build or tests can't enforce). If a rule fails all six criteria *and* shows no discrimination in A/B tests, it is dead weight. Delete it. The **vltansky agents-md-evals** repo on GitHub is the concrete tool for this — it runs comparative evals on AGENTS.md variants and reports which rules actually move the pass rate. Their headline finding from early adopters: most files are **80–95% redundant**, and trimming improves agent behavior rather than degrading it.

**The `@import` pattern.** Modern AGENTS.md and CLAUDE.md files support an `@<filename>` directive that pulls in referenced files inline — for example, `@verification/rules.md` loads that file's content as if it were written directly. For verification handoff this enables hierarchical instruction organization: a root `AGENTS.md` imports `verification/AGENTS.md` (mutation floors, test commands) and `contracts/AGENTS.md` (broker URL, provider verification commands) without loading all of it into every context. The agent sees the import directive in the root file and follows it when entering that directory. This is the [progressive-disclosure](../context-engineering/instruction-layer.md) principle applied to instruction files themselves — discovery tokens are cheap, full content loads only when relevant. It also solves an accrual problem: when each domain area owns its verification rules in a co-located file, the rules stay close to what they govern and get updated by whoever is working in that area, rather than accumulating in an omnibus root file that nobody owns.

For a broader survey of the file hierarchy and how CLAUDE.md, AGENTS.md, and per-skill files compose, see the **Data Science Collective "Complete Guide to CLAUDE.md and AGENTS.md 2026"**. For the specific pattern of emitting a `HANDOVER.md` at session end so the next session can pick up mid-task, see the **Ilyas Ibrahim Mohamed "4-Step Protocol That Fixes Claude Code's Context Amnesia"** piece — it codifies a handover prompt template worth stealing wholesale. The four steps: (1) summarize the end goal and why it matters, (2) document the approach being taken and why alternatives were ruled out, (3) list steps completed and their outcomes (including failed attempts — these are the most valuable), and (4) describe the current failure or open question the next session should start from. A HANDOVER.md built from these four elements gives the incoming session correct information about trajectory, not just state. Dexter Horthy's ACE-FCA writeup arrives at the same structure through a different route — their compaction prompt outputs the same four elements: goal, approach, steps done, current failure. The format is converging because it matches what agents actually need to avoid re-doing work and re-learning constraints that were already discovered.

For a more ambitious direction, **Mozilla.ai's Cq** ("Stack Overflow for AI coding agents," March 2026, 225 HN points) is an early shot at cross-session knowledge sharing as *infrastructure* rather than per-repo convention — a queryable knowledge base with verified answer snippets and provenance, meant to give agents a curated source to pull from rather than regurgitating training-set patterns. If agents can pull verified, versioned answers from Cq instead of synthesizing from training data, the surface area for confidently-wrong outputs shrinks. None of these have replaced AGENTS.md, but the space is moving.
